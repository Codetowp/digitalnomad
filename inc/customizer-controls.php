...
/*-----------------------------------------------------------------------------------*/
/*  digitalnomad Customizer Controls
/*-----------------------------------------------------------------------------------*/

class Digitalnomad_Misc_Control extends WP_Customize_Control 
	{
...
class Digitalnomad_Textarea_Custom_Control extends WP_Customize_Control
	{
		public function render_content() 
		{
			?>
...
class Digitalnomad_Theme_Support extends WP_Customize_Control 
	{
		public function render_content() 
		{
			echo wp_kses_post( 'Upgrade to <a href="#">Digitalnomad Plus</a> to be able to change the section order and styling!', 'saint' );
		}
	}
 ...
 */
function Digitalnomad_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = digitalnomad_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  digitalnomad_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
class Digitalnomad_Editor_Custom_Control extends WP_Customize_Control
{
    /**
 ...
 */
class Digitalnomad_Alpha_Color_Control extends WP_Customize_Control {
 ...
 * @access public
 */
class Digitalnomad_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'digitalnomad_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
            $this->max_item = absint( $args['max_item'] );
        }

        $this->changeable =  isset(  $args['changeable'] ) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset(  $args['default_empty_title'] ) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__( 'Item', 'digitalnomad' );
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    #>
                                        ...
                                        <# }  else if ( field.type == 'icon'  ) { #>
                                            ...
                                            <a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'digitalnomad' ); ?></a>
                                        <# }  #>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php _e( 'Remove', 'digitalnomad' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php _e( 'Close', 'digitalnomad' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
...
function digitalnomad_enqueue_editor(){
...
class Digitalnomad_Editor_Scripts
{
    /**
     * Enqueue scripts/styles.
     *
...
function digitalnomad_customizer_control_scripts(){
    wp_enqueue_media();
...
add_action( 'customize_controls_enqueue_scripts', 'digitalnomad_customizer_control_scripts', 99 );
add_action( 'customize_controls_enqueue_scripts', array( 'Digitalnomad_Editor_Scripts', 'enqueue' ), 95 );
