 ...
 */
function digitalnomad_sanitize_text_trim( $value ) {
	if ( empty( $value ) ) {
		return '';
	}
	return trim( $value );
 ...
 */
function digitalnomad_sanitize_social_buttons( $value ) {
	if ( ! is_array( $value ) ) {
		return array();
	}
 ...
 */
class Digitalnomad_Sanitize_Select {
	/**
	 * Select keys
	 * @var array
	 */
	 ...
	 */
	public function Digitalnomad_Sanitize_Select( $keys, $default_value = '' ) {
		$this->keys = $keys;
